...
Construct a binary classification network that operates on eeg and audio.
Note how many dimensions are hardcoded based on assumed input dimensions.
 
"""
__author__ = 'Greg Ciccarelli'
__date__ = 'September 27, 2018'
...
class NN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.i2o_l4 = nn.Linear(1 * (input_size), output_size, bias=True)

        # May Need a bias to get from audio (not zero mean) to eeg (zero mean)
        self.i2o_l1only = nn.Linear(input_size + hidden_size, output_size, bias=True)    
        
        
        self.bn_l0 = nn.BatchNorm1d(19)
